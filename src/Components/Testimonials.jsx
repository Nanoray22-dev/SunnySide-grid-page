  ...
  const user = [
    {
      img: imgEmily,
      content:
        "We put our trust in Sunnyside and they delivered, making sure our needs were met and deadlines were alyways hit.",
      name: "Emily R.",
  ...
  return (
    <>
      <div className="text-center py-[5rem] px-[3rem] lg:pt-[7rem] lg:px-[5rem] xl:py-[10rem] xl:px-[15rem]">
        <h3 className="uppercase text-2xl tracking-[.40em] text-[#a7abae]">
          Client Testimonials
        </h3>
          ...
          {user.map((data) => (
            <div key={data.name} className="pt-[6rem]">
              <img
                src={data.img}
                alt={data.name}
                className="inline-block rounded-full w-[100px]"
              />
              <p className="py-[3rem] text-gray-500  px-[2rem]">
                {data.content}
              </p>
              <h4 className="font-[900] text-[#23303e] font-Fraunces ">
                {data.name}
              </h4>
